...
import { ghostStartPosition } from "../types/position";
...
const GhostIcon = () => (
  ...
  >
    <g>
      <g>
        <path d="M128.1,129.7c-4.9-4.5-8.8-8.1-12.8-11.8c-3.4,3.7-6.7,7.3-10.3,11.3c-3.4-3.7-6.8-7.4-10.3-11.2
...
const Ghost = (props) => {
  ...
  const [position, setPosition] = useState(ghostStartPosition);
  const [direction, setDirection] = useState(DIRECTION.LEFT);
  const [color, setColor] = useState(props.color);
  const [changeDirectionWaitingTime, setChangeDirectionWaitingTime] = useState(0);
  ...
  function move() {
    if (gameStatus === GAME_STATUS.IN_PROGRESS) {
      if (changeDirectionWaitingTime > 4) {
        ...
        setChangeDirectionWaitingTime((prev) => prev + 1);
      }
          ...
          default:
            newPosition = {
              top: Math.min(
                ...
                window.innerHeight - props.size - props.border - props.topScoreBoard
              ),
              left: currentLeft,
            };
        }
